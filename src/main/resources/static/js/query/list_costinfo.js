...
async function fetchCostInfoData(queryParams, page, pageSize) {
    try {
        const params = {
            page: page,
            pageSize: pageSize,
        ...
        // 使用fetch API发送POST请求到后端获取数据接口，这里假设后端接口地址为/api/listCostInfo，需根据实际情况调整
        const response = await fetch('/api/listCostInfo', {
            method: 'POST',
            headers: {
...
function updatePageLinks(totalRecord, page, pageSize) {
    ...
    if (page!== 1) {
        ...
        firstPageLink.href = `listCostInfo.html?Page=1`;
...
window.addEventListener('load', async () => {
    ...
    const pageSize = 6;
    const costInfoData = await fetchCostInfoData(queryParams, page, pageSize);
    const { totalRecord, data } = costInfoData;
    ...
    searchForm.addEventListener('submit', async (event) => {
        ...
        const newCostInfoData = await fetchCostInfoData(newQueryParams, 1, pageSize);
        const { totalRecord: newTotalRecord, data: newData } = newCostInfoData;
